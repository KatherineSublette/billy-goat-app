...
@Component({
  selector: 'app-completed-trips-page',
  templateUrl: './completed-trips-page.component.html',
  styleUrls: ['./completed-trips-page.component.css']
})
export class CompletedTripsPageComponent implements OnInit {
  private jwtUser: User = JSON.parse(sessionStorage.getItem("user"));
  private user: User;
  private guest: Guest;
  private trips: Job[] = [];
  private noGuide: "Unclaimed";
  private displayedColumns: string[] = ['name', 'resort', 'guide', 'date'];
  ...
  refreshTrips(): void {
    let filter = "?guestId=" + this.guest.id.toString() + "&completed=true";
    console.log(filter)
